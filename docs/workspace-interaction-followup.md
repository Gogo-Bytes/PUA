...
## 2026-09-23：移除新建前资源提示

移除 PendingChatPane 和新建会话弹窗的“检测到项目资源”卡片与检查等待门槛。启动沿用 Pi 已保存/默认资源策略，不自动批准项目扩展。草稿只在输入 `@` 或 `/` 时按需读取建议资源；检查失败不阻止发送。弹窗同时在提交层拒绝空白路径。模型/思考的会话前接口、多标签各类内容的真实宿主接入仍未完成，不能把入口存在描述为功能已完整交付。
