...
新对话模型/思考选择已接入；Files 已完成只读目录浏览及 256 KB UTF-8 文本预览。Terminal、Browser、Side chat 仍需真实宿主能力接入，不能把入口存在描述为功能已完整交付。
...
3. 右侧 Files 已实现会话目录浏览与受限文本预览；Terminal/Browser/Side chat、分支操作、提交推送、subagents/后台进程仍待接入。完整 unchanged 展开需要新的两侧内容授权接口，当前变更不含这些内容。
