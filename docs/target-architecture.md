...
首批已集中 Desktop IPC channel/DTO/schema、统一 main sender → parser seam、打包 sandbox preload，并建立包含 tests/fixtures 的 TS/TSX 类型检查及隔离 IPC/生命周期 smoke 的 `verify`（`.mjs` fixture 未被 TypeScript 检查）。后续结构批次又退休旧 `src/main` 与 shared 顶层契约，归位 app/preload/workers/platform/modules/renderer features，并将 main create/attachment 用例从具体 process Adapter 去耦。`SessionProcessAdapter` 作为单 registry、多窄 Port 的物理资源 owner 有意保留，只有 composition root 构造；不为目录对称拆出共享 registry 或浅代理。阶段 0 的 formatter/完整 lint、完整产品 smoke、preview export 清理、阶段 2 完整桌面验收与完整阶段 3–5 仍未完成；阶段 3 已落地 send/attachment、runtime stop/activity/waiting 与 stream/tool/history，但未获本轮运行验证。生产 UI 第一阶段已接入 `UIProvider`、semantic tokens、单一主题解析和显式样式入口，第二阶段已统一 Icon 与 Dialog 并删除旧顶层 seam，第三阶段已将 ProjectNav 与 SessionTabs 迁入 Workspace Feature，第四阶段已将 InspectorHeader 与 FileRow 迁入 Change Review Feature，第五阶段已将 ToolExecutionCard 迁入 Conversation Feature，第六阶段已将 ChatMessage 迁入 Conversation Feature，第七阶段已将 Composer 迁入 Conversation Feature 并删除 `renderer/modules` 的运行时聚合入口与样式，第八阶段已将共用 ContentView seam 迁入 `renderer/features/content`，第九阶段已将顶层 `review.css` 迁入 Change Review Feature 并退休旧路径，第十阶段已按原 cascade 顺序拆分并退休顶层 `styles.css`，由 `app/production.css` 统一装配 app/Feature stylesheets；第十一阶段已将迁移契约归档至 `renderer/ui` 并删除 `renderer/modules` 目录。生产 UI 路径迁移已收敛，运行验收仍待授权。组件/demo 保护集合继续保留并按授权改动更新精确 hash。具体结构 checkpoint 与验证限制见 [现行架构](architecture.md#非-ui-结构迁移-checkpoint)。
...
`SessionCoordinator` 负责编排 use case；`SessionOwnershipPolicy` 以显式结果表达允许或拒绝及原因；具体 utility process 位于 Adapter。renderer 的 launch/form/rename presentation 已统一归入 `renderer/features/sessions`，不取得后端生命周期权威。
...
### 当前非 UI 结构 checkpoint

用户授权范围内的非 UI 源码结构迁移已经完成：进程入口、领域 owner、平台 Adapter、IPC 契约、main composition 与 renderer Feature 均可按当前树定位，旧路径和兼容胶水已删除。目标目录是职责示意，不要求创建空 Workspace backend/application、Terminal 转发 application class、单实现 infrastructure/ports 目录，也不要求拆分持有唯一 registry 的 `SessionProcessAdapter`。

该 checkpoint 未执行 typecheck、自动测试、build、Electron/Pi/lifecycle smoke 或应用，不满足阶段 2–5 的行为验收和发布条件。下一阶段按用户顺序处理 UI/design-system/`renderer/modules`/Demo 的生产收敛；formatter、完整 lint、运行验证和跨平台 release matrix 仍是独立门禁。不得将此状态写成全量目标架构完成。
...
`app/useDesktopPresentation` 持窗口 boot/error/Settings open 和 refresh/publish，复用 useTheme；`features/sessions` 统一持有 `useSessionLaunchController`、mount-local form/inspection/trust、`useSessionPresentation` 与两个 Dialog，分别保持 create continuation 和 rename 提交身份/回写/openProject。`features/workspace/useSessionInput` 持按 ID 草稿、活 TerminalHandle registry、search 与命令/引用路由；`useWorkspace` 仍唯一拥有 sessions/selection/events/close。palette 与 Settings draft 原 owner 不变，所有跨 feature caller 仍经 index。
...
此前条件解冻只授权生产架构职责归位；后续获批的生产 UI 第一阶段仅接入 `UIProvider`、semantic tokens、主题解析和显式 production stylesheet，并保持 preview fixture 隔离。该阶段提交后按用户指令执行 `npm run dev`，其中 production build 成功并启动 Electron；未运行测试、独立 typecheck、smoke、浏览器检查或视觉/行为验收。第二阶段删除旧顶层 `Icon.tsx` 与 `Modal.tsx`，并让 Extension UI 复用唯一 Dialog；Terminal 静态 palette 仍归 Terminal Feature。第三阶段将 ProjectNav 与 SessionTabs 从 `renderer/modules` 迁入 Workspace Feature，保留生产选择/关闭/overflow/键盘与独立 Rename Dialog owner，并让隔离预览继续用本地 rename/add callback；第四阶段将 InspectorHeader 与 FileRow 迁入 Change Review Feature 并接入 GitPanel，不移动 Git 状态、scope、selected path、刷新 generation 或 diff 请求 owner；第五阶段将 ToolExecutionCard 迁入 Conversation Feature 并接入生产 ToolCard，保留原生 details/summary、失败自动展开及 tool arguments/images/output owner；第六阶段将 ChatMessage 迁入 Conversation Feature 并接入生产 MessageView，保留 transcript shell 与 ContentView/Desktop action 边界；第七阶段将 Composer 迁入 Conversation Feature，并以只提供原 `.composer` DOM seam 的 transcript variant 接入 ChatPane，不移动 textarea ref、IME、draft revision、附件 token、发送/引导/后续/停止锁或异步 continuation，同时删除 `renderer/modules` 的运行时入口与样式。第八阶段将顶层 `ContentView.tsx` 迁入 `renderer/features/content`，Conversation 与 Change Review 经 Feature index 共用，且不改变 clipboard/external-link Desktop Client seam、Markdown/流式修补或 SourceView DOM。第九阶段将顶层 `review.css` 原字节迁入 `features/change-review/change-review-production.css`，生产入口与隔离 Workspace Preview 改用新路径，不改变 selector、cascade 顺序或 GitPanel DOM。第十阶段按原 selector/token 顺序将顶层 `styles.css` 拆入 app、Workspace、Terminal、Conversation 与 Content stylesheets；`production.css` 成为唯一生产入口，隔离 Workspace Preview 也改用该入口，旧路径删除并加入退休保护。第十一阶段将迁移契约归档至 `renderer/ui/production-integration.md`，更新开发路由与保护集合，并删除已无运行时源码的 `renderer/modules` 目录。第二至第十一阶段只做静态检查，未重新运行或重启应用；真实桌面/原生对话与跨平台验收仍未完成。

完成条件：

- `App.tsx` 只承担组合，不拥有各 Feature 工作流；
- Workspace、Session、Conversation、Terminal、Change Review、Preferences UI 各自可定位；
- Theme、Icon 与 Dialog 已收敛；布局体系完成收敛；
- 草稿、附件、后台事件、滚动和焦点无回归。
...
| 已删除 `src/renderer/features/session-launch` 与 Workspace 下的 Session rename/action 文件；现 `renderer/features/sessions` | Session launch/form/rename presentation，公开入口为 feature index；不取得后端生命周期权威 |
| 已删除 `src/renderer/session-state.ts` 与顶层 `WorkspaceNavigation.tsx`；现 `renderer/features/workspace` | 唯一窗口投影/选择、导航、草稿/handle 与 reference path 格式化；不持 launch/rename 或后端生命周期权威 |
| 已删除顶层 `src/renderer/ContentView.tsx`；现 `src/renderer/features/content` | Conversation 与 Change Review 共用的 Desktop-aware content presentation；经 Feature index 暴露，不下沉无宿主依赖的 `renderer/ui` |
| `src/renderer/ui` | 唯一通用 UI 系统，清除业务与 preview 内容 |
| 已删除 `src/renderer/modules` | 迁移契约归档于 `renderer/ui/production-integration.md`；不得重建模糊生产聚合目录 |

此表记录当前归属并指导后续维护，不是机械的一对一搬运清单。目标树中的可选空层不构成待办；只有新的职责或真实变化点出现时才创建。移动前先确认所有权和 Interface，移动后删除旧路径。
