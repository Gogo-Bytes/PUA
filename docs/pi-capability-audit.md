...
- 任务生命周期：关闭默认归档；详情设置可恢复或二次确认永久删除。永久删除先验证 Pi identity/cwd 与普通文件边界，再同文件系统 rename 为 tombstone、提交 PUA 索引删除，最后 unlink；中途失败会独立补偿路径和索引，不允许静默丢失历史。
...
| PI-RPC-12 | 自定义 skill / prompt template 管理 | Pi CLI runtime 解析 `--skills`、`--prompt-templates`，resource loader 可加载；Skill 使用行首 `/skill:name`，Prompt Template 使用行首 `/name` | 部分接入 | `@` Skill tooltip（仅行首，避免把 Pi 不会执行的内联提及伪装成能力）、原生 `/模板` 候选、命令面板和项目启动前资源/信任检查已接入；懒草稿通过安全的目录名/文件名预览候选，真正执行仍以 Pi 会话 `get_commands` 为权威；资源实际执行仍由 Pi 持有 |
...
- **项目草稿附件**：Pi 的附件登记依赖已创建的 Conversation identity。PUA 因此先用主进程文件选择器暂存路径，首次发送创建 Pi 会话后再调用受控 `registerAttachments`，随后只把 Pi 返回的附件 token 交给 prompt；不会在未创建会话时伪造 Pi attachment ID。
...
- 用户已确认分支展示方案：项目下保留主会话，分支作为可折叠缩进子节点。Pi 标记为可分支的用户消息节点提供原生 Fork 操作；助手、工具和系统节点只作为历史树信息展示，不伪造“直接切换”能力。此前“Codex 式 A/C”是设计提案标签，不作为 Codex 已实测行为的证据。
