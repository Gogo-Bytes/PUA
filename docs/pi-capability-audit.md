...
当前 PUA worker protocol 已覆盖 prompt/steer/followUp、stop、clear queue、extension response、rename、fork、clone、模型/Thinking 查询与切换、compact、自动策略设置、队列策略设置和部分会话查询；明确协议缺口仍包括 `abort_retry`、`switch_session`、`get_entries`、`export_html`、`bash` 和 `abort_bash`。这些命令不应通过任意字符串透传，必须逐项加入白名单 DTO、响应校验、超时和生命周期测试。
