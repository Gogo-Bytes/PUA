...
- 全本地历史：覆盖活动、休眠和归档任务；结果携带 task/session/entry 定位，点击后恢复对应任务并定位到消息。需要定义扫描上限、索引缓存和归档/永久删除后的失效策略。
- 活动会话：只查询当前已启动的 Pi RPC；无需读取本地 JSONL，但无法覆盖重启后未激活或归档的历史。

无论选择哪一项，都不搜索凭据、环境变量、工具参数之外的隐藏扩展状态，也不把搜索结果当成新的 Pi 会话或分支。
