...
### 阶段 K：工作台视觉层级与 Codex 对照（已接入）

- 顶部工作区栏保持三列比例、项目标题和当前草稿状态；更多操作使用真实任务操作，右侧检查器入口使用独立的面板切换，不添加不存在的 Share 或云端任务按钮。
- 项目树保留 PUA 的二级会话列表、折叠、置顶、关闭、兼容终端和 Pi 分支入口；项目行与会话行分别表达“进入新草稿”和“恢复已有任务”，不会为了贴合 Codex 把两种生命周期合并。
- Git 检查器继续消费真实 `gitStatus/fileDiff` 快照，视觉上采用 Codex Environment 卡片的圆角、分组和密度；变更数量、文件路径、差异、引用和刷新仍是 PUA 本地 Git 语义。
- Pi 工具执行卡片使用真实 tool 生命周期、状态和耗时，补齐可展开层级、边界和状态密度；不生成 Codex 专属的“Edited N files”、审查提交或远程 Subagent 数据。
- Composer 继续暴露 Pi 的附件、命令、`@` skill 候选、模型、Thinking、压缩、统计、排队、停止和发送；未确认 Pi 等价能力的麦克风、分享、云端任务预算不在视觉对照中添加。
