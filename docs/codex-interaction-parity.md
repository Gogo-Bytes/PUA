...
当前采用已确认的“全部本地历史”范围：覆盖活动、休眠和归档任务。搜索对 PUA 索引登记的 Pi JSONL 做主进程受限扫描，每个文件最多读取 12 MiB，单次最多返回 40 条；结果只携带任务、项目、entry、角色、时间和安全文本摘要，不暴露 `sessionFile` 或原始 JSONL。点击归档结果会先恢复原生 Pi 会话，再进入任务并尝试定位匹配消息；项目草稿不属于历史搜索结果。
