...
给你已经在用的 **Pi coding agent** 一个桌面工作空间。当前为 **0.1.0 终端优先预览版**，不是能力缩水的聊天代理，也还不是完整的原生聊天 GUI。
...
- 命令面板、终端历史搜索、字号调整、多行中文草稿、文件引用插入。
...
- 这版是桌面终端工作台。原生聊天气泡、结构化工具卡片、会话树侧栏、diff 审查、任务通知尚未实现。
...
- 技术选型中的外部官网实时核验受当前浏览工具缺失影响，已明确标注，不能将本地研究说成完整在线调研。

详见 [研究与兼容矩阵](research.md)、[架构说明](docs/architecture.md)、[验证记录](docs/validation.md)。
