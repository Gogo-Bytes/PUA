...
describe('desktop IPC contract', () => {
  it('retains exactly the legacy invoke/send/event whitelist and unique channels', () => {
    ...
    expect(Object.keys(invokeChannels)).toHaveLength(37);
    ...
    expect(values.map(value => value.replace('desktop:', '')).sort()).toEqual([
      ...
      'external', 'project', 'git-status', 'file-diff', 'clipboard-read', 'clipboard-write', 'chat-fork', 'chat-clone', 'chat-session-stats', 'chat-auto-settings', 'chat-compact', 'chat-auto-compaction-set', 'chat-auto-retry-set', 'write', 'resize', 'ack', 'event',
    ].sort());
