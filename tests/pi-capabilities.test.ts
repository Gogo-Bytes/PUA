...
describe('Pi capability registry', () => {
  ...
  it('records the implemented Pi paths as integrated and leaves unresolved paths explicit', () => {
    for (const id of ['conversation.stream', 'conversation.queue', 'conversation.compact', 'conversation.retry', 'conversation.retryAbort', 'conversation.stats', 'session.fork', 'session.tree', 'session.clone', 'session.rename', 'model.list', 'model.select', 'thinking.select', 'resources.skills', 'resources.promptTemplates', 'extensions.ui'] as const) {
      expect(piCapabilities.find(item => item.id === id)?.status, id).toBe('integrated');
    }
    for (const id of ['session.entries'] as const) {
