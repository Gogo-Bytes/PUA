...
try {
  ...
  await popup.getByRole('button', { name: /main.*本地分支/ }).click();
  await popup.getByRole('textbox', { name: '新分支名称' }).fill('feature/preview');
  await popup.getByRole('button', { name: '创建并切换' }).click();
  await popup.getByRole('button', { name: /feature\/preview.*本地分支/ }).waitFor();
  await popup.getByRole('button', { name: /^main/ }).click();
  await popup.getByRole('button', { name: /main.*本地分支/ }).waitFor();
  await popup.getByRole('button', { name: /main.*本地分支/ }).click();
  await popup.locator('.environment-branch-row').filter({ hasText: 'feature/preview' }).locator('button').last().click();
  const deleteDialog = page.getByRole('dialog').filter({ hasText: '将尝试安全删除' });
