...
export function desktopIPCFake() {
  ...
  const getGitStatus = vi.fn<ChangeReview['snapshot']>().mockResolvedValue({ root: '/repo', branch: 'main', capturedAt: 'now', files: [{ path: 'new\nname', originalPath: 'old', index: 'R', worktree: ' ' }] }); const getGitBranches = vi.fn<ChangeReview['branches']>().mockResolvedValue(['main']); const switchGitBranch = vi.fn<ChangeReview['switchBranch']>(); const createGitBranch = vi.fn<ChangeReview['createBranch']>(); const deleteGitBranch = vi.fn<ChangeReview['deleteBranch']>(); const getGitWorktrees = vi.fn<ChangeReview['worktrees']>().mockResolvedValue({ current: '/repo', worktrees: [{ path: '/repo', head: 'abc', branch: 'main', current: true }] }); const createGitWorktree = vi.fn<ChangeReview['createWorktree']>().mockResolvedValue({ current: '/repo', worktrees: [{ path: '/repo', head: 'abc', branch: 'main', current: true }] }); const deleteGitWorktree = vi.fn<ChangeReview['deleteWorktree']>().mockResolvedValue({ current: '/repo', worktrees: [{ path: '/repo', head: 'abc', branch: 'main', current: true }] }); const getFileDiff = vi.fn<ChangeReview['preview']>().mockResolvedValue({ text: 'patch', kind: 'diff', truncated: false }); const inspectProjectResources = vi.fn().mockResolvedValue({ hasResources: false, paths: [] });
  ...
  registerDesktopIPC({ ipcMain, dialog, shell, clipboard, requireCurrent: holder.requireCurrent, rendererURL: fake.webContents.mainFrame.url, preferences, changeReview: { snapshot: getGitStatus, preview: getFileDiff, branches: getGitBranches, switchBranch: switchGitBranch, createBranch: createGitBranch, deleteBranch: deleteGitBranch, worktrees: getGitWorktrees, createWorktree: createGitWorktree, deleteWorktree: deleteGitWorktree }, inspectProjectResources, listSessionFiles, readSessionFile, browserViews: browserViews as never });
