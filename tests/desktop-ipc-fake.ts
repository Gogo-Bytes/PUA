...
export function desktopIPCFake() {
  ...
  const getGitStatus = vi.fn<ChangeReview['snapshot']>().mockResolvedValue({ root: '/repo', branch: 'main', capturedAt: 'now', files: [{ path: 'new\nname', originalPath: 'old', index: 'R', worktree: ' ' }] }); const getGitBranches = vi.fn<ChangeReview['branches']>().mockResolvedValue(['main']); const switchGitBranch = vi.fn<ChangeReview['switchBranch']>(); const createGitBranch = vi.fn<ChangeReview['createBranch']>(); const deleteGitBranch = vi.fn<ChangeReview['deleteBranch']>(); const getFileDiff = vi.fn<ChangeReview['preview']>().mockResolvedValue({ text: 'patch', kind: 'diff', truncated: false }); const inspectProjectResources = vi.fn().mockResolvedValue({ hasResources: false, paths: [] });
