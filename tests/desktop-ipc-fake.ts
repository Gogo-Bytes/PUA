...
export function desktopIPCFake() {
  ...
  registerDesktopIPC({ ipcMain, dialog, shell, clipboard, requireCurrent: holder.requireCurrent, rendererURL: fake.webContents.mainFrame.url, preferences, changeReview: { snapshot: getGitStatus, preview: getFileDiff }, inspectProjectResources, listSessionFiles, readSessionFile, browserViews: browserViews as never });
  ...
  return { fake, window, holder, capabilities, invokes, sends, store, runtime, preferences, resolveRuntime, validateChatArguments, dialog, shell, clipboard, getGitStatus, getFileDiff, inspectProjectResources, listSessionFiles, readSessionFile, browserViews, event, call, rawCall, bridge, client, listeners, emit };
}
