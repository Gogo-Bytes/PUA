...
describe('real App → SettingsDialog → Fake Desktop characterization', () => {
  it('opens, chooses, parses, publishes runtimeError without closing, corrects and saves theme/bootstrap in place', async () => {
    ...
    vi.mocked(window.desktop.chooseFile).mockReturnValueOnce(picker.promise);
    ...
    expect(document.documentElement.dataset.theme).toBe('light'); expect(window.desktop.savePreferences).not.toHaveBeenCalled();
    ...
    expect(within(dialog).getByRole('alert').textContent).toMatch(/^SyntaxError:/); expect(window.desktop.savePreferences).not.toHaveBeenCalled();
    ...
    const sentAgain = vi.mocked(window.desktop.savePreferences).mock.calls[1][0];
    ...
    expect(window.desktop.bootstrap).toHaveBeenCalledTimes(1); expect(window.desktop.startSession).not.toHaveBeenCalled(); expect(window.desktop.createSession).not.toHaveBeenCalled(); expect(window.desktop.closeSession).not.toHaveBeenCalled();
  ...
  it('keeps NewSessionDialog → settings jump and uses saved bootstrap on a later launch', async () => {
    ...
    vi.mocked(window.desktop.savePreferences).mockResolvedValueOnce({ ...boot, runtimeError: undefined, runtime: { executable: '/fixed', args: [], source: '/fixed' }, preferences: { ...boot.preferences, recentProjects: ['/saved-default'] } });
  ...
  it.each(['success', 'runtimeError', 'reject'] as const)('preserves late old-save %s after closing and opening another settings dialog', async outcome => {
    ...
    expect(window.desktop.savePreferences).toHaveBeenCalledTimes(1);
  });
});
