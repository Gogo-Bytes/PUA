...
beforeEach(() => {
  ...
  window.desktop = { chooseFile: vi.fn().mockResolvedValue(null), savePreferences: vi.fn(async (preferences: Preferences) => ({ ...boot, preferences })) } as unknown as typeof window.desktop;
...
describe('SettingsDialog public view characterization (Fake Desktop)', () => {
  it('preserves initial JSON formatting, default theme, DOM semantics and range Number conversion', async () => {
    ...
    expect(window.desktop.savePreferences).not.toHaveBeenCalled(); submit(); await flush();
    expect(window.desktop.savePreferences).toHaveBeenCalledWith({ ...boot.preferences, fontSize: 22 });
  ...
  it('keeps mount-only dirty draft/args but live runtime display and new callbacks after props change', async () => {
    ...
    expect(window.desktop.savePreferences).toHaveBeenCalledWith({ ...boot.preferences, piPath: '/dirty', args: ['--dirty'] });
  ...
  it('preserves the initial preferences reference rather than cloning or freezing it', async () => {
    ...
    submit(); await flush();
    const sent = vi.mocked(window.desktop.savePreferences).mock.calls[0][0];
  ...
  it('merges late picker completion into the latest fields; same-key picks follow completion order', async () => {
    ...
    vi.mocked(window.desktop.chooseFile).mockReturnValueOnce(first.promise).mockReturnValueOnce(second.promise);
    ...
    submit(); await flush(); expect(window.desktop.savePreferences).toHaveBeenCalledWith(expect.objectContaining({ piPath: '/first', nodePath: '/typed-node', theme: 'dark' }));
  });
  it('preserves picker cancel, failure String and unchanged error on later success', async () => {
    vi.mocked(window.desktop.chooseFile).mockResolvedValueOnce(null).mockRejectedValueOnce({ toString: () => 'picker denied' }).mockResolvedValueOnce('/new-node');
    render(<SettingsDialog {...props} />); choose(); await flush(); expect(input('Pi 路径').value).toBe('/pi');
    choose(); await flush(); expect(screen.getByRole('alert').textContent).toBe('picker denied');
    choose(1); await flush(); expect(input('Node.js 路径').value).toBe('/new-node'); expect(screen.getByRole('alert').textContent).toBe('picker denied');
    expect(window.desktop.savePreferences).not.toHaveBeenCalled();
  });
  it('catches synchronous picker throw too', async () => {
    vi.mocked(window.desktop.chooseFile).mockImplementation(() => { throw new Error('sync picker'); });
  ...
  it.each(['{}', 'null', '12', '"value"', '[1]', '[null]', '["ok",false]'])('keeps the existing array/string-only client check for %s', async args => {
    ...
    expect(window.desktop.savePreferences).not.toHaveBeenCalled(); expect(props.onSave).not.toHaveBeenCalled();
  });
  it('reports native JSON parse failure, then clears error and enters busy before host save', async () => {
    const pending = deferred<Bootstrap>(); vi.mocked(window.desktop.savePreferences).mockReturnValueOnce(pending.promise);
    ...
    expect(window.desktop.savePreferences).toHaveBeenCalledWith(expect.objectContaining({ args: ['', '--mode', 'rpc'] }));
    await act(async () => pending.reject({ toString: () => 'host rejects' })); expect(screen.getByRole('alert').textContent).toBe('host rejects'); expect(saveButton().disabled).toBe(false);
  });
  it('catches host synchronous throw and allows explicit correction/retry', async () => {
    vi.mocked(window.desktop.savePreferences).mockImplementationOnce(() => { throw new Error('sync save'); });
  ...
  it('publishes the exact result before reading runtimeError, including callback mutation and same-continuation close', async () => {
    ...
    vi.mocked(window.desktop.savePreferences).mockReturnValueOnce(pending.promise);
  ...
  it('publishes runtimeError results yet keeps dialog editable and resets busy, then closes on successful retry', async () => {
    ...
    vi.mocked(window.desktop.savePreferences).mockResolvedValueOnce(result);
  ...
  it('does not add a lock for duplicate synthetic submits and applies each captured result in completion order', async () => {
    ...
    vi.mocked(window.desktop.savePreferences).mockReturnValueOnce(first.promise).mockReturnValueOnce(second.promise);
    render(<SettingsDialog {...props} />); submit(); change('Pi 路径', '/second'); submit();
    expect(window.desktop.savePreferences).toHaveBeenCalledTimes(2);
    const calls = vi.mocked(window.desktop.savePreferences).mock.calls;
  ...
  it('retains captured callbacks/values during save even after props change and draft editing', async () => {
    ...
    const sent = vi.mocked(window.desktop.savePreferences).mock.calls[0][0]; sent.nodePath = '/host-mutated';
  ...
  it.each(['resolve', 'reject'] as const)('does not affect a new dialog after late picker %s; late failures are still stringified', async outcome => {
    const pending = deferred<string | null>(); vi.mocked(window.desktop.chooseFile).mockReturnValueOnce(pending.promise);
    ...
    expect(input('Pi 路径').value).toBe('/new-dialog'); expect(screen.queryByRole('alert')).toBeNull(); expect(toString).toHaveBeenCalledTimes(outcome === 'reject' ? 1 : 0);
  });
});
