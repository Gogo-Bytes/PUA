...
function harness(emit = vi.fn(), overrides: Omit<CompositionDependencies, 'createAdapter'> = {}) {
  ...
  const adapter = {
    ...
    getAvailableModels: vi.fn<ProcessAdapter['getAvailableModels']>().mockResolvedValue([]), getAvailableThinkingLevels: vi.fn<ProcessAdapter['getAvailableThinkingLevels']>().mockResolvedValue([]),
    setModel: vi.fn<ProcessAdapter['setModel']>().mockResolvedValue(undefined), setThinkingLevel: vi.fn<ProcessAdapter['setThinkingLevel']>().mockResolvedValue(undefined),
    setAutoCompaction: vi.fn<ProcessAdapter['setAutoCompaction']>().mockResolvedValue(undefined), setAutoRetry: vi.fn<ProcessAdapter['setAutoRetry']>().mockResolvedValue(undefined),
    setSteeringMode: vi.fn<ProcessAdapter['setSteeringMode']>().mockResolvedValue(undefined), setFollowUpMode: vi.fn<ProcessAdapter['setFollowUpMode']>().mockResolvedValue(undefined),
    getSessionStats: vi.fn<ProcessAdapter['getSessionStats']>().mockResolvedValue({ userMessages: 0, assistantMessages: 0, toolCalls: 0, toolResults: 0, totalMessages: 0, tokens: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 }, cost: 0 }),
    getAutoSettings: vi.fn<ProcessAdapter['getAutoSettings']>().mockResolvedValue({ autoCompaction: true, autoRetry: true, steeringMode: 'one-at-a-time', followUpMode: 'one-at-a-time' }),
    compact: vi.fn<ProcessAdapter['compact']>().mockResolvedValue(undefined),
