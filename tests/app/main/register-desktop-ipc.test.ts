...
const samples: { [K in RequestMethod]: RequestArgs<K> } = {
  ...
  stopChat: ['id'], openProject: ['id'], gitStatus: ['id'], gitBranches: ['id'], switchGitBranch: ['id', 'feature/test'], createGitBranch: ['id', 'feature/new'], deleteGitBranch: ['id', 'feature/old'], gitWorktrees: ['id'], createGitWorktree: ['id', 'feature/worktree'], deleteGitWorktree: ['id', '/repo-worktree'], commitGitChanges: ['id', 'commit message'], pushGitChanges: ['id'], writeClipboard: ['clipboard'], listSessionFiles: ['id', ''], readSessionFile: ['id', 'README.md'],
...
describe('real registerDesktopIPC with Fake Electron and closed business dependencies', () => {
  it('registers every declared invoke and send exactly once and calls every real handler', async () => {
    ...
    expect(results.commitGitChanges).toEqual({ root: '/repo', branch: 'main', capturedAt: 'now', files: [] });
    expect(results.pushGitChanges).toEqual({ root: '/repo', branch: 'main', capturedAt: 'now', files: [] });
    ...
    expect(h.commitGitChanges).toHaveBeenCalledExactlyOnceWith('/fake/project', 'commit message'); expect(h.pushGitChanges).toHaveBeenCalledExactlyOnceWith('/fake/project');
  ...
  it('every untrusted invoke/send rejects before parser and all injected filesystem/runtime/dialog/Git/core effects', () => {
    ...
    for (const spy of [h.store.write, h.resolveRuntime, h.validateChatArguments, h.dialog.showOpenDialog, h.dialog.showMessageBox, h.shell.openPath, h.shell.openExternal, ...Object.values(h.clipboard), h.getGitStatus, h.getGitBranches, h.switchGitBranch, h.createGitBranch, h.deleteGitBranch, h.getGitWorktrees, h.createGitWorktree, h.deleteGitWorktree, h.commitGitChanges, h.pushGitChanges, h.getFileDiff, h.inspectProjectResources, h.listSessionFiles, h.readSessionFile, ...Object.values(h.browserViews), ...Object.values(h.capabilities.session), ...Object.values(h.capabilities.conversation), ...Object.values(h.capabilities.terminal), h.capabilities.createSession, h.capabilities.registerChatAttachments]) expect(spy).not.toHaveBeenCalled();
