...
function fixture() {
  ...
  for (const directory of ['src/renderer/ui', 'src/renderer/modules', 'tests/component-preview']) mkdirSync(path.join(root, directory), { recursive: true });
  const files = ['src/renderer/ui/example.tsx', 'src/renderer/modules/README.md', 'tests/component-preview/style.css', 'tests/component-example.test.tsx'];
...
it('keeps the real frozen file set and content identical to the supplied manifest', () => {
  ...
  expect(Object.keys(manifest)).toHaveLength(44);
...
it.each(['change', 'add', 'delete', 'rename', 'symlink'])('detects protected file %s in every frozen area', operation => {
  for (let index = 0; index < 4; index++) {
    ...
    if (operation === 'add') writeFileSync(index === 3 ? path.join(root, 'tests/component-added.test.tsx') : absolute + '.new', 'new');
