...
describe('inspector data boundary', () => {
  it('shows actual hunk counts and numbers, distinguishes scope, and references without sending', async () => {
    ...
    expect(window.desktop.fileDiff).toHaveBeenCalledWith('s1', 'tracked.ts', 'worktree');
    ...
    fireEvent.click(screen.getByRole('button', { name: '复制差异输出' })); await waitFor(() => expect(window.desktop.writeClipboard).toHaveBeenCalledWith(expect.stringContaining('@@ -8,2 +8,2 @@')));
    fireEvent.click(screen.getByRole('button', { name: /暂存区 1/ })); expect(screen.queryByRole('button', { name: /notes.md/ })).toBeNull();
    fireEvent.click(screen.getByRole('button', { name: 'tracked.ts M' })); await waitFor(() => expect(window.desktop.fileDiff).toHaveBeenLastCalledWith('s1', 'tracked.ts', 'index'));
  ...
  it.each([
    ...
    vi.mocked(window.desktop.gitStatus).mockResolvedValue({ root: '/repo', branch: 'main', capturedAt: '2026-01-01T00:00:00Z', files: [{ path: 'conflict.txt', index, worktree }] });
    vi.mocked(window.desktop.fileDiff).mockResolvedValue({ kind: 'diff', text, truncated });
    ...
    await waitFor(() => expect(window.desktop.writeClipboard).toHaveBeenCalledWith(text));
  });
  it('renders preview/source solely from untracked text returned by fileDiff, never reconstructs a tracked file', async () => {
    ...
    vi.mocked(window.desktop.fileDiff).mockResolvedValue({ kind: 'untracked', truncated: true, text: source });
    const { container } = render(<GitPanel sessionId="s1" onClose={() => {}} onReference={() => {}} />);
    ...
    fireEvent.click(screen.getByRole('button', { name: '复制文件快照' })); await waitFor(() => expect(window.desktop.writeClipboard).toHaveBeenCalledWith(source));
    expect(window.desktop.fileDiff).toHaveBeenCalledTimes(1);
  });
  it('ignores stale scope requests and visibly reports failures; binary/symlink results have no file preview', async () => {
    ...
    vi.mocked(window.desktop.fileDiff).mockImplementationOnce(() => new Promise(resolve => { resolveOld = resolve; })).mockResolvedValueOnce({ kind: 'symlink', truncated: false, text: '符号链接 → /outside' });
    ...
    vi.mocked(window.desktop.gitStatus).mockRejectedValueOnce(new Error('git unavailable'));
...
describe('real content actions', () => {
  it('keeps clipboard and external link errors visible instead of reporting success', async () => {
    vi.mocked(window.desktop.writeClipboard).mockRejectedValue(new Error('clipboard denied'));
    vi.mocked(window.desktop.openExternal).mockRejectedValue(new Error('blocked protocol'));
  ...
  it('retains unknown tool arguments, images, output and source boundaries in the real disclosure', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '复制工具输出' })); await waitFor(() => expect(window.desktop.writeClipboard).toHaveBeenCalledWith('one\ntwo'));
  });
});
