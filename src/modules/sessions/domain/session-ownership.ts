...
export interface PreparedSession extends SessionIntent { id: string; cwd: string; title: string }
...
export const SessionOwnershipPolicy = {
  ...
  reserve(intent: SessionIntent, sessions: readonly SessionSnapshot[], shuttingDown: boolean): SessionResult {
    ...
    const live = sessions.filter(session => session.lifecycle.phase !== 'exited');
