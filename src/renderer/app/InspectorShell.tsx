...
export function InspectorShell({ task, runtime, changes, onClose, onOpenProject, onRename, onArchive, onTogglePinned }: { task: SessionInfo; runtime: RuntimeInfo | null; changes: ReactNode; onClose(): void; onOpenProject(): void; onRename(): void; onArchive(): void | Promise<void>; onTogglePinned?(): void | Promise<void> }) {
  ...
  return <aside className="inspector-shell" aria-label="任务检查器">
    ...
    <div className="inspector-shell-body">
      <div hidden={view !== 'task'}><TaskDetailsPanel task={task} runtime={runtime} onOpenProject={onOpenProject} onRename={onRename} onArchive={onArchive} onTogglePinned={onTogglePinned}/></div>
