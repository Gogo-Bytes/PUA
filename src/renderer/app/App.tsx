...
import { TerminalOutlet } from '../features/terminal';
...
export function App() {
  ...
  const terminalStage = useRef<HTMLDivElement>(null);
  ...
  return <UIProvider theme={theme}><div className={`workspace ${boot?.platform === 'darwin' ? 'workspace-darwin' : ''}`}>
    ...
    <ResizableWorkspace rightSize={{ initial: 560, min: 280, max: 1000 }} leftToggleRef={leftPanelToggle} leftOpen={projectPanelOpen} onLeftOpenChange={setProjectPanelOpen} rightToggleRef={panelToggle} rightOpen={reviewOpen} onRightOpenChange={setReviewOpen} hideToolbar
      ...
      right={<SidePanelHost key={activeId ?? project} panels={panels} task={active} cwd={project} runtime={boot?.runtime ?? null} terminalDockRef={terminalDock} onClose={closeReview} onOpenProject={() => { if (active) sessionActions.openProject(active.id); }} onRename={sessionActions.beginRename} onArchive={() => { if (active) return workspace.closeSession(active.id); }} onTogglePinned={() => { if (active) return workspace.setSessionPinned(active.id, !active.pinned); }} onClone={() => { if (active) return sessionActions.cloneTask(active.id); }} onError={desktopPresentation.reportError} changes={active && <GitPanel key={active.id} theme={theme} sessionId={active.id} onClose={() => panels.close('review')} onReference={input.reference}/>} />}>
    ...
    <TerminalOutlet sessions={sessions} activeId={activeId} platform={boot?.platform ?? ''} fontSize={boot?.preferences.fontSize ?? 14} theme={theme} docked={reviewOpen && panels.active === 'terminal' && active?.kind === 'terminal'} stageRef={terminalStage} dockRef={terminalDock} onReady={input.onTerminalReady} onExit={workspace.markSessionExited} onError={desktopPresentation.reportError}/>
