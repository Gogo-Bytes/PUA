...
import { HistorySearchDialog, ProjectSidebar, TaskToolbar, readWorkspaceView, writeWorkspaceView } from '../features/workspace';
...
export function App() {
  ...
  return <UIProvider theme={theme}><div className="workspace">
    <ResizableWorkspace hideToolbar rightToggleRef={panelToggle} rightOpen={reviewOpen} onRightOpenChange={setReviewOpen}
      ...
      {active && <TaskToolbar task={active} inspectorToggleRef={panelToggle} inspectorOpen={reviewOpen} onToggleInspector={() => setReviewOpen(open => !open)} onSelectProject={() => navigation.selectProject(active.cwd)} onOpenProject={() => sessionActions.openProject(active.id)} onRename={sessionActions.beginRename} onArchive={() => workspace.closeSession(active.id)} onTogglePinned={() => workspace.setSessionPinned(active.id, !active.pinned)}>
        {active.kind === 'chat' && <Button variant="ghost" onClick={() => launch.open({ cwd: active.cwd, kind: 'terminal', mode: 'new' })}>兼容终端</Button>}
        {active.kind === 'terminal' && <><Button variant="ghost" onClick={() => launch.open({ cwd: active.cwd, kind: 'terminal', mode: 'new' })}>兼容终端</Button><Button variant="ghost" onClick={() => input.toggleSearch()}>搜索</Button><Button variant="ghost" onClick={() => void input.chooseTerminalReferences()}>＋ 文件引用</Button></>}
      </TaskToolbar>}
