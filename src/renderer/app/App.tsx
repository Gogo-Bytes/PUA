...
export function App() {
  ...
  return <UIProvider theme={theme}><div className="workspace">
    <header className="workspace-heading"><strong>PUA</strong><span className="ui-meta">工作台</span><Button variant="ghost" onClick={palette.openFromSidebar} disabled={!active}><Icon name="search"/>搜索与命令<kbd>⌘ K</kbd></Button><IconButton label="桌面设置" icon="settings" variant="ghost" onClick={desktopPresentation.showSettings} disabled={!boot}/></header>
    <ResizableWorkspace rightToggleRef={panelToggle} rightOpen={reviewOpen} onRightOpenChange={setReviewOpen}
      ...
      left={<div className="workspace-navigation"><ProjectNavigation sessions={sessions} recentProjects={boot?.preferences.recentProjects ?? []} project={project} onSelect={navigation.selectProject} onAdd={() => launch.open()}/><div className="sidebar-bottom"><span className="ui-meta">{boot?.runtime ? '本机 Pi' : '尚未连接 Pi'}</span></div></div>}
      ...
      <header className="topbar"><SessionTabs sessions={projectSessions} activeId={activeId} onSelect={navigation.selectSession} onClose={id => void workspace.closeSession(id)} onRename={navigation.renameSession} onAdd={launch.openInProject} addDisabled={!boot} showOverflow groupKey={project ?? ''} labels={{ title: '项目会话', add: '新建会话', all: '全部会话', menu: '当前项目全部会话', close: title => `关闭 ${title}`, rename: { hint: '双击或 F2 重命名', input: value => `重命名 ${value}`, save: '保存', cancel: '取消', empty: '名称不能为空', failed: '重命名失败，请重试' } }} /></header>
