...
SidePanelHost 现在是右侧多标签宿主；无标签显示 Review、Terminal、Browser、Files、Side chat。Files 已接入只读会话目录浏览和 UTF-8 文本预览，draft 尚无 session ID 时显示可用条件；Terminal、Browser、Side chat 仍是说明入口，不伪造宿主能力。文件 IPC 只接受 session ID + 相对路径，由主进程从 session capability 解析 cwd；路径不允许逃逸、不跟随符号链接，列表跳过特殊文件，预览限制 256 KB 并拒绝二进制/无效 UTF-8。tab 列表按 session/draft 隔离，切 tab 保持已打开 Review 挂载，切会话重置内容投影；全关后回到入口。面板宽度默认 560、最小 280、最大 1000，仍保留 center 360 最小约束与响应式隐藏。
