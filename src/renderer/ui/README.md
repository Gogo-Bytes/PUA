...
- **空间跟随内容**：短通知一行，不强制标题另起一行，操作紧邻正文；显式标题或长错误可展开完整正文。Dialog 按内容自适应，Composer 保留紧凑工具区与可滚动附件条。缩小留白不能裁掉错误或隐藏可操作链接；按用户要求不使用焦点外框，键盘定位保留文字下划线、图标色或列表高亮反馈。常规目标至少 24px，必要时通过间距保证可操作性。
