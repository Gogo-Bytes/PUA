...
const clamp = (value: number, max: number, min = MIN) => Math.max(min, Math.min(max, value));
/** Container-first fitting: right temporarily hides before left; saved preferences survive narrow viewports. */
export function ResizableWorkspace({ left, right, children, leftOpen, onLeftOpenChange, rightOpen, onRightOpenChange, toolbarActions, leftToggleRef, rightToggleRef, motionControls = false, hideToolbar = false, rightSize, labels }: { left: ReactNode; right: ReactNode; children: ReactNode; leftOpen?: boolean; onLeftOpenChange?(open: boolean): void; rightOpen?: boolean; onRightOpenChange?(open: boolean): void; toolbarActions?: ReactNode; leftToggleRef?: Ref<HTMLButtonElement>; rightToggleRef?: Ref<HTMLButtonElement>; motionControls?: boolean; hideToolbar?: boolean; rightSize?: { initial: number; min: number; max: number }; labels?: { left: string; right: string; show: string; hide: string; resize(side: 'left' | 'right'): string; compact: string; hint: string } }) {
  ...
  const rightMin = rightSize?.min ?? MIN;
  const rightMax = rightSize?.max ?? MAX;
  ...
  const [containerWidth, setContainerWidth] = useState(1200), [widths, setWidths] = useState({ left: 280, right: rightSize?.initial ?? 300 }), [localShown, setLocalShown] = useState({ left: true, right: true });
  const focusToggle = (side: Side) => {
    const external = side === 'left' ? leftToggleRef : rightToggleRef;
    (toggles.current[side] ?? (external && typeof external === 'object' ? external.current : null))?.focus();
  };
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      for (const side of ['left', 'right'] as const) {
        const minimum = CENTER_MIN + (side === 'right' ? rightMin : MIN) + HANDLE + (side === 'right' && shown.left ? MIN + HANDLE : 0);
        const panel = side === 'left' ? leftRef.current : rightRef.current;
        if (width < minimum && (panel?.contains(document.activeElement) || separators.current[side] === document.activeElement)) focusToggle(side);
      }
  ...
  const visible = { left: shown.left && containerWidth >= CENTER_MIN + MIN + HANDLE, right: shown.right && containerWidth >= CENTER_MIN + rightMin + HANDLE + (shown.left ? MIN + HANDLE : 0) };
  const available = Math.max(0, containerWidth - CENTER_MIN - (visible.left ? HANDLE : 0) - (visible.right ? HANDLE : 0));
  const actual = { left: visible.left ? Math.min(widths.left, available - (visible.right ? rightMin : 0)) : 0, right: 0 };
  actual.right = visible.right ? Math.min(widths.right, available - actual.left) : 0;
  const maxFor = (side: Side) => Math.min(side === 'right' ? rightMax : MAX, available - actual[side === 'left' ? 'right' : 'left']);
  const canShow = (side: Side) => containerWidth >= CENTER_MIN + (side === 'right' ? rightMin : MIN) + HANDLE + (side === 'right' && shown.left ? MIN + HANDLE : 0);
  function stopDrag() {
  ...
  function moveDrag(event: PointerEvent<HTMLDivElement>) {
    ...
    const width = clamp(current.width + (event.clientX - current.x) * (current.side === 'left' ? 1 : -1), maxFor(current.side), current.side === 'right' ? rightMin : MIN);
  ...
  return <div ref={root} className="ui-workspace" onKeyDown={event => {
    ...
    {!hideToolbar && <div className="ui-workspace-toolbar">{(['left', 'right'] as const).map(side => <Button ref={node => { toggles.current[side] = node; const target = side === 'left' ? leftToggleRef : rightToggleRef; if (typeof target === 'function') target(node); else if (target) target.current = node; }} key={side} variant="ghost" aria-expanded={visible[side]} aria-disabled={!visible[side] && !canShow(side) || undefined} title={!visible[side] ? '空间足够时自动恢复面板' : undefined} onClick={() => { if (!visible[side] && !canShow(side)) return; stopDrag(); toggle(side, !visible[side]); }}><Icon name="panel"/>{visible[side] ? text.hide : text.show} {text[side]}</Button>)}{motionControls && <details className="ui-workspace-test-controls"><summary>测试控制 · 动效</summary><Button variant="ghost" onClick={() => setReplayCount(count => count + 1)}>Replay motion</Button></details>}{toolbarActions}<span className="ui-meta">{containerWidth < 736 ? text.compact : text.hint}</span></div>}
