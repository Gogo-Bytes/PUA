...
export function ResizableWorkspace({ left, right, children }: { left: ReactNode; right: ReactNode; children: ReactNode }) {
  ...
  const [containerWidth, setContainerWidth] = useState(1200), [widths, setWidths] = useState({ left: 224, right: 256 }), [shown, setShown] = useState({ left: true, right: true });
  ...
  return <div ref={root} className="ui-workspace">
    <div className="ui-workspace-toolbar">{(['left', 'right'] as const).map(side => <Button ref={node => { toggles.current[side] = node; }} key={side} variant="ghost" aria-expanded={visible[side]} aria-disabled={!visible[side] && !canShow(side) || undefined} title={!visible[side] ? 'Saved width returns when the container has enough room.' : undefined} onClick={() => { if (!visible[side] && !canShow(side)) return; stopDrag(); setShown(previous => ({ ...previous, [side]: !visible[side] })); }}><Icon name="panel"/>{visible[side] ? 'Hide' : 'Show'} {side} panel</Button>)}<details className="ui-workspace-test-controls"><summary>测试控制 · 动效</summary><Button variant="ghost" onClick={() => setReplayCount(count => count + 1)}>Replay motion</Button></details><span className="ui-meta">{containerWidth < 736 ? 'Compact · panels return when space permits' : 'Drag edges · arrows to resize'}</span></div>
