...
export function DropdownMenu({ label, items, onAction, icon, iconOnly = false, disabled = false, className = '', triggerRef }: {
  label: string; items: Choice[]; onAction(value: string): void; icon?: Parameters<typeof Icon>[0]['name']; iconOnly?: boolean; disabled?: boolean; className?: string; triggerRef?: Ref<HTMLButtonElement>;
}) {
  const container = useOverlayContainer();
  return <div className="ui-popup-anchor"><MenuRoot>
    <MenuTrigger ref={triggerRef} className={`ui-button ui-button-ghost ${iconOnly ? 'ui-icon-button' : ''} ${className}`} aria-label={label} disabled={disabled || !items.some(item => !item.disabled)}>{!iconOnly && label}<Icon name={icon ?? 'down'}/></MenuTrigger>
