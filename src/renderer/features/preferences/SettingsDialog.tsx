  ...
  return <Dialog open title="桌面设置" closeLabel="关闭对话框" closeOnBackdrop={false} onClose={onClose}>
    <form onSubmit={event => { event.preventDefault(); save(); }}>
      ...
      <div className="input-row">
        <input aria-label="Pi 路径" value={value.piPath} onChange={event => setValue({ ...value, piPath: event.target.value })} />
        <button type="button" onClick={() => void pick('piPath')}>选择…</button>
      </div>
      <label>Node.js 可执行文件（可选）</label>
      <div className="input-row">
        <input aria-label="Node.js 路径" value={value.nodePath} onChange={event => setValue({ ...value, nodePath: event.target.value })} />
        <button type="button" onClick={() => void pick('nodePath')}>选择…</button>
      </div>
      <label>附加 Pi CLI 参数 · JSON 字符串数组</label>
      <input className="full-input mono" aria-label="Pi CLI 参数" value={args} onChange={event => setArgs(event.target.value)} />
