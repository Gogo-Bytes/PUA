...
export function PendingChatPane({ cwd, runtimeAvailable, value, onValueChange, stagedAttachmentPaths = [], onStagedAttachmentPathsChange, onStart, onSettings }: Props) {
  ...
  useEffect(() => {
    ...
    setInspection(undefined); setTrust('default'); setAttachmentPaths(stagedAttachmentPaths);
  ...
  return <section className="pending-chat-pane" aria-label="新对话">
    ...
    <div className="pending-composer-wrap">
      <Composer suggestions={suggestions} conversationKey={`draft:${cwd}`} editorRef={editor} value={value} onValueChange={onValueChange} attachments={attachments} onAddAttachments={() => void addAttachments()} onRemoveAttachment={id => setAttachmentPaths(current => current.filter(path => path !== id))} onSend={async submission => { if (inspecting) throw new Error('正在检查项目资源，请稍后再发送'); if (inspectionError) throw new Error(inspectionError); await onStart(submission.value, trust, attachmentPaths); setAttachmentPaths([]); }} labels={{ message: '发送消息', placeholder: '描述任务、粘贴内容或添加文件…', hint: 'Enter 发送 · ⇧Enter 换行', send: '发送消息', addAttachments: '添加附件', attachments: '附件', removeAttachment: name => `移除 ${name}`, queued: count => `${count} 已排队`, failed: '无法开始对话', error: '操作失败' }}/>
    </div>
  </section>;
}
