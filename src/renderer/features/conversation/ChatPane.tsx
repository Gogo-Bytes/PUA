...
export function ChatPane({ session, active, draft, onDraftChange, onError, onCommands, onTerminalRecovery }: Props) {
  ...
  return <section className={`chat-pane ${active ? 'active' : ''}`} aria-hidden={!active} data-session-id={session.id}>
    ...
    <div className="composer-area">
      ...
      {!!skills.length && <div className="skill-menu" role="listbox" aria-label="技能建议" onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); setSkillDismissed(true); textarea.current?.focus(); } if (['ArrowDown', 'ArrowUp'].includes(event.key)) { event.preventDefault(); const items = [...event.currentTarget.querySelectorAll<HTMLButtonElement>('[role=option]')]; const index = items.indexOf(event.target as HTMLButtonElement); items[(index + (event.key === 'ArrowDown' ? 1 : -1) + items.length) % items.length]?.focus(); } }}> {skills.map(command => <Button key={command.name} role="option" aria-selected="false" onClick={() => { const before = draft.slice(0, draft.length - (skillMatch?.[1].length ?? 0)); const name = command.name.replace(/^skill:/, ''); changeDraft(`${before.replace(/@$/, '')}/skill:${name} `); setSkillDismissed(true); textarea.current?.focus(); }}><code>@{command.name.replace(/^skill:/, '')}</code><span>{command.description || '@ Pi 技能'}</span></Button>)}</div>}
      <div className="chat-runtime-controls"><select aria-label="模型" value={state.model ? `${state.model.provider}/${state.model.id}` : ''} onFocus={loadModels} onChange={event => { const [provider, ...rest] = event.target.value.split('/'); void desktopClient.setChatModel(session.id, provider, rest.join('/')).catch(error => onError(String(error))); }}>{!state.model && <option value="">模型</option>}{models.map(model => <option key={`${model.provider}/${model.id}`} value={`${model.provider}/${model.id}`}>{model.name || model.id}</option>)}</select><select aria-label="Thinking Level" value={state.thinkingLevel || ''} onChange={event => { if (event.target.value) void desktopClient.setChatThinkingLevel(session.id, event.target.value).catch(error => onError(String(error))); }}><option value="">Thinking</option>{['off','minimal','low','medium','high'].map(level => <option key={level} value={level}>{level}</option>)}</select></div>
