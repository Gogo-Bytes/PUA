...
export function App() {
  ...
  const close = async (id: string) => {
    try {
      ...
      if (activeId === id) setActiveId(sessions.find(session => session.id !== id)?.id ?? null);
  ...
  return <div className="workspace">
    <aside className="sidebar">
      <div className="brand"><span className="brand-icon">π</span><div>Pi Desktop<small>YOUR AGENT. YOUR WAY.</small></div><span className="version">0.1</span></div>
      ...
      <nav className="sessions" aria-label="会话">
        ...
        {sessions.map(session => <div key={session.id} className={`session-row ${session.id === activeId ? 'selected' : ''}`}>
    ...
    <main>
      ...
      <div className={`terminal-stage ${!active ? 'hidden' : ''}`}>
