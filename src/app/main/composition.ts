...
export function composeMain(emit: (event: SessionEvent) => void, dependencies: CompositionDependencies = {}) {
  ...
  return {
    ...
    conversation: conversation as Pick<ConversationApplication, 'send' | 'stop' | 'respond' | 'rename' | 'fork' | 'getAvailableModels' | 'getAvailableThinkingLevels' | 'setModel' | 'setThinkingLevel' | 'removeAttachment'>,
