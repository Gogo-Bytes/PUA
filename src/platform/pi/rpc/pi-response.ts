...
export interface PiResponseData {
  ...
  fork: { text: string; cancelled: boolean }; clone: { cancelled: boolean }; switch_session: { cancelled: boolean }; set_model: unknown; set_thinking_level: unknown; set_auto_compaction: unknown; set_auto_retry: unknown; abort_retry: unknown; abort_bash: unknown; set_steering_mode: unknown; set_follow_up_mode: unknown; compact: unknown; export_html: { path: string };
}
...
function validData<C extends PiCommand['type']>(command: C, data: unknown): data is PiResponseData[C] {
  switch (command) {
    ...
    case 'prompt': case 'abort': case 'set_session_name': case 'set_model': case 'set_thinking_level': case 'set_auto_compaction': case 'set_auto_retry': case 'abort_retry': case 'abort_bash': case 'set_steering_mode': case 'set_follow_up_mode': case 'compact': return true;
  }
}
